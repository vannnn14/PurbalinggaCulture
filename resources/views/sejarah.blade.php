      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 pt-8 pb-12 grid grid-cols-1 lg:grid-cols-2 gap-10">
        <article class="space-y-6">
          ...
          <div class="bg-[#F8EED6] p-6 rounded-xl shadow-sm ring-1 ring-black/5 text-[15px] leading-relaxed space-y-4">
            <p><strong>Kyai Arsantaka</strong> (Arsakusuma), putra Bupati Onje II, menjadi sosok sentral. Ia diangkat
              anak oleh Kyai Wanakusuma di Masaran dan pada 1740–1760 menjabat demang di Pagendolan di bawah Tumenggung
              Dipayuda I.</p>
            <p>Dalam <em>Perang Jenar</em> (bagian dari perang Mangkubumen), Kyai Arsantaka berpihak pada Kadipaten
              Banyumas yang membela Paku Buwono. Atas jasanya, Adipati Banyumas R.T. Yudanegara mengangkat putranya,
              <strong>Kyai Arsayuda</strong>, menjadi menantu.
            </p>
            <p>Kelak, Kyai Arsayuda menjadi Tumenggung Karangwelas bergelar <strong>R.T. Dipayuda III</strong> dan
              memindahkan pusat pemerintahan ke desa Purbalingga—yang berkembang menjadi pusat kabupaten hingga kini.
            </p>
          </div>
        </article>
        <figure class="flex justify-center items-start">
  <div class="w-full max-w-xl aspect-[4/3] bg-transparent rounded-xl overflow-hidden shadow">
    <img src="/images/gambarsejarah.png" alt="Ilustrasi sejarah Purbalingga" class="w-full h-full object-contain"
  ...
  <figcaption class="sr-only">Ilustrasi pemindahan pusat pemerintahan ke Purbalingga.</figcaption>
    ...
    <section class="bg-[#1c1005] py-6 md:py-10">
      ...
      <div x-data="{
          ...
          go(i){ this.current = i }
        }" class="relative max-w-7xl mx-auto px-4 sm:px-6" aria-roledescription="carousel"
            ...
            <template x-for="(slide, i) in slides" :key="i">
              <div class="min-w-full relative">
                <div class="w-full aspect-[16/9] md:h-[520px]">
                  <img :src="slide.img" :alt="slide.text" class="w-full h-full object-cover" loading="eager"
                    decoding="async">
                </div>
                <div
          ...
          <div class="absolute bottom-3 md:bottom-4 w-full flex justify-center gap-2">
            <template x-for="(slide, i) in slides" :key="`dot-${i}`">
              <button @click="go(i)"
                ...
                :class="current===i ? 'bg-white' : 'bg-white/40'" :aria-label="`Ke slide ${i+1}`"
    ...
    <section id="tokoh" class="py-12 bg-[#FFECD1]/50">
      {{-- Header --}}
      <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="flex items-center justify-between">
          ...
          <a href="/tokoh" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#8b5e34]
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 text-center">
        <p class="text-gray-600 mt-5">Beberapa tokoh besar dari Purbalingga yang memberi warna bagi sejarah dan budaya.
        </p>

        {{-- Grid tokoh (4 highlight) --}}
        <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {{-- Jenderal Soedirman --}}
          <a href="/tokoh/jenderal-soedirman" class="relative rounded-xl overflow-hidden shadow-lg group flex flex-col">
            <div class="w-full h-48 bg-[#1c1005]">
            ...
            <div class="flex-1 flex flex-col justify-between bg-black/60 px-4 py-2 text-white">
              <h3 class="text-lg font-semibold">Jenderal Soedirman</h3>
              <p class="text-sm text-gray-200">Panglima TNI, Pahlawan Nasional</p>
            </div>
          </a>

          {{-- Mayjend Soengkono --}}
          <a href="/tokoh/mayjend-soengkono"
              class="relative rounded-xl overflow-hidden shadow-lg group flex flex-col">
            <div class="w-full h-48 bg-[#1c1005]">
              <img src="/images/soengkono.jpeg" alt="Mayjend Soengkono"
                class="w-full h-full object-contain group-hover:scale-105 transition duration-500">
            </div>
            <div class="flex-1 flex flex-col justify-between bg-black/60 px-4 py-2 text-white">
              <h3 class="text-lg font-semibold">Mayjend Soengkono</h3>
              <p class="text-sm text-gray-200">Tokoh Pertempuran Surabaya 1945</p>
            </div>
          </a>

          {{-- Usman Janatin --}}
          <a href="/tokoh/usman-janatin" class="relative rounded-xl overflow-hidden shadow-lg group flex flex-col">
            <div class="w-full h-48 bg-[#1c1005]">
              <img src="/images/usmanjanatin.jpeg" alt="Usman Janatin"
                class="w-full h-full object-contain group-hover:scale-105 transition duration-500">
            </div>
            <div class="flex-1 flex flex-col justify-between bg-black/60 px-4 py-2 text-white">
              <h3 class="text-lg font-semibold">Usman Janatin</h3>
              <p class="text-sm text-gray-200">Pahlawan Dwikora</p>
            </div>
          </a>

          {{-- Sukatani --}}
          <a href="/tokoh/indro-warkop" class="relative rounded-xl overflow-hidden shadow-lg group flex flex-col">
            <div class="w-full h-48 bg-[#1c1005]">
              <img src="/images/sukatani.jpeg" alt="Indro Warkop"
                class="w-full h-full object-contain group-hover:scale-105 transition duration-500">
            </div>
            <div class="flex-1 flex flex-col justify-between bg-black/60 px-4 py-2 text-white">
              <h3 class="text-lg font-semibold">Sukatani</h3>
              <p class="text-sm text-gray-200">Musik punk & kritik sosial modern</p>
            </div>
          </a>
        </div>
      </div>
